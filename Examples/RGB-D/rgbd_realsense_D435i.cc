...
int main(int argc, char** argv) {
    ...
    // rs2::align allows us to perform alignment of depth frames to others
    // frames
    ...
    auto imu_callback = [&](const rs2::frame& frame) {
        ...
        if (rs2::frameset fs = frame.as<rs2::frameset>()) {
            ...
            if (profile_changed(pipe.get_active_profile().get_streams(),
                ...
                // If the profile was changed, update the align object, and also
                // get the new device's depth scale
                pipe_profile = pipe.get_active_profile();
                align_to = find_stream_to_align(pipe_profile.get_streams());
                align = rs2::align(align_to);
            }

            // Align depth and rgb takes long time, move it out of the
            // interruption to avoid losing IMU measurements
            fsSLAM = fs;
...
rs2_stream find_stream_to_align(
    const std::vector<rs2::stream_profile>& streams) {
    // Given a vector of streams, we try to find a depth stream and another
    // stream to align depth with. We prioritize color streams to make the view
    // look better. If color is not available, we take another stream that
    // (other than depth)
