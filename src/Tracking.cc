...
void Tracking::Track() {
    ...
    if (mState == NOT_INITIALIZED) {
        ...
        if (bOK) {
            ...
            // Triangulation analysis between current frame and last keyframe
            if (mpLastKeyFrame &&
                (mSensor == System::RGBD || mSensor == System::IMU_RGBD)) {
                vector<pair<int, int>> vTriangulationPairs;
                ...
                for (int i = 0; i < mCurrentFrame.N; i++) {
                    ...
                    if (pMP && !mCurrentFrame.mvbOutlier[i] && !pMP->isBad()) {
                        // Check if this MapPoint was observed in the last
                        // keyframe
                        ...
                        if (observations.count(mpLastKeyFrame)) {
                            int keyframe_idx =
                                get<0>(observations[mpLastKeyFrame]);
                            vTriangulationPairs.push_back(
                                make_pair(i, keyframe_idx));
                        }
                    }
                }

                // Triangulate using existing feature correspondences
                for (auto& correspondence : vTriangulationPairs) {
                    int curr_idx = correspondence.first;
                    int kf_idx = correspondence.second;

                    // Get keypoints (reusing already extracted features)
                    cv::KeyPoint kp_curr = mCurrentFrame.mvKeysUn[curr_idx];
                    cv::KeyPoint kp_kf = mpLastKeyFrame->mvKeysUn[kf_idx];
                    ...
                    // Get camera poses
                    Eigen::Matrix<float, 3, 4> Tcw_curr =
                        mCurrentFrame.GetPose().matrix3x4();
                    Eigen::Matrix<float, 3, 4> Tcw_kf =
                        mpLastKeyFrame->GetPose().matrix3x4();

                    // Triangulate using geometric method
                    ...
                    if (GeometricTools::Triangulate(x_curr, x_kf, Tcw_curr,
                        ...
                        if (depth_rgbd > 0) {
                            // Get 3D point from RGB-D
                            Eigen::Vector3f x3D_rgbd;
                            if (mCurrentFrame.UnprojectStereo(curr_idx,
                                                              x3D_rgbd)) {
